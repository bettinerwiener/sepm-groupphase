...
public class NewsEndpoint {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    @ApiOperation(value = "Add an image to a news entry", authorizations = {@Authorization(value = "apiKey")})
    public NewsDto updateImage(@RequestParam("image") MultipartFile image, @PathVariable("id") Long id)  {
        return this.newsMapper.newsToNewsDto(this.newsService.updateWithImage(id, image));
    }
    ...
    public NewsDto getNewsById(@PathVariable("id") Long id) {
        News result = this.newsService.findById(id);
        return this.newsMapper.newsToNewsDto(result);
    }
